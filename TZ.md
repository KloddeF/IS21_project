...
Оглавление:
    ...
    5.Требования к интерфейсу
        ...
        5.3 Меню начать игру (Обновлено)
        5.4 Меню управления лобби (Новое)
    6.Требования к внутреигровым механикам
        6.1 Удар 
        6.2 Выстрел
    7.Переход между экранами (Обновлено)
...
4.4Требования к экранам приложения
    ...
    4.3 Экран арены фарма
         ...
         4.3.1. Общее описание
   		* Часть сцены, где происходит фарм монстров. Единственная часть сцены, где спавнятся монстры.
        ...
        4.3.3. Сценарии развития
            ...
            Основной поток:
            * Герой приходит на "Арену фарма". 
            * Монстры начинают появляться.
            * Герой убивает всех монстров.
            * Начинается фаза закупки.

            4.3.3.2. Герой погибает от монстров.
            Условие: монстры убили героя, у героя было минимум 2 жизни
            Основной поток:
            * Герой теряет одну жизнь.
            * Герой переходит в режим наблюдателя.
            * После убийства всех монстров, другими игроками, игрок возраждается.
            ...
            * Игрок видит сообщение о проигрыше.
            ...
            4.3.3.4 В игре остается единственный игрок хотя бы с одной жизнью.
        ...
        4.3.4. Функциональные требования
            * Все герои находятся на одной а
            ...
            * Герой не может выйти за пределы "видимого экрана" арены фарма, до убийства всех монстров. (Экран становится неподвижным, видимая часть сцены фиксируется).
            * После убийства всех монстров игрок может выйти за пределы арены фарма и пройти дальше.
            * Игроки не могут нанести какой либо урон друг другу.
            * При появлении на арене фарма, игрок должен увидеть на центре экрана сообщение с номером волны. (Волна N.) N - номер волны.
            * При смерте игрока, система должна списать у него одну жизнь и переместить игрока в режим наблюдателя.
            * Монстры появляются в невидимой для игроков части арены(за видимым экраном), а потом выходят на видимую для игроков часть арены.
            * Монстры начинают появляться спустя 3 секунды, после того как все игроки окажутся на арене фарма.
            * Как только каждый из героев умрет, либо игроки победят всех монстров начинается "Время закупки".
            * Если в игре остается один игрок, игра заканчивается.
            * Победившйи игрок перемещается в меню.
    ...
    5.Требования к интерфейсу
        5.1. Худ игрока
            ...
            * Худ игрока не должен отображаться в меню. А также во время выбора, покупки класса и в магазине внутреигровых предметов. В остальное время худ отображается всегда.
            ...
            Визуальное оформление:
                * Представляет из себя 4 полупрозрачный ячейки. Ячейки отображаются в левом нижнем углу.
            Технические требования:
                ...
                * Игрок может переключаться между ячейками инвентаря с помощью клавиш: "1", "2", "3", "4" - соответсвенно, либо при нажатии на определенную ячейку.
        ...
        5.3. Меню начать игру
            ...
            * Текст "Название комнаты" - Лобби игрока {имя игрока}
        ...
        5.4 Меню управления лобби
            5.4.1. Общее описание
            * Всплывающее меню в котором игрок создавший лобби, может управлять им. А обычные игроки - покинуть игру.

            5.4.2. Визуальное оформление
            * Меню оформлено в виде развернутого свитка.
            * Фон - затемненный экран.
            * Страница свитка (1 вариант):
                * Отцентрованный текст "Название лобби"
                * Список игроков:
                    * Имя игрока
                    (Если игрок явялется хостом, к его имени должна быть добавлена надпись - " (Хост) ")
                * Красная кнопка "Покинуть лобби"
            * Страница свитка (2 вариант)
                * Копия 1 варианта, добавляются кнопки "Исключить игрока" (красные крестики), напротив имен игроков.
                * Напротив имени Хоста (игрока создавшего лобби), крестика нет.
            * В правом верхнем углу правой страницы Кнопка "Закрыть" - красный крестик.

            5.4.3. Функциональные требования
            * Текст "Название комнаты" - Лобби игрока {имя игрока}
            * Все игроки должны отображаться в списке игроков.
            * Если игрок покидает лобби, он должен быть удлаен из смписка игроков.
            * Хост (игрок создавший лобби), должен иметь приписку "(Хост)".
            * Кнопка "Покинуть игру" - всегда активна.
            * При нажатии кнопки "Покинуть лобби", игрок должен быть перемещен в меню, и удален из лобби.
            * Если хост покидает игру, лобби должно быть удалено, а все игроки перемещены в меню с сообщением об ошибке "Хост покинул игру."
            * Кнопка "Закрыть" - всегда активна.
            * При нажатии на кнопку "Закрыть" - меню Начать игру должно быть закрыто.
            * Кнопка "Исключить игрока" - всегда активна. (отображается только у Хоста)
            * При нажатии кнопки "Исключить игрока", соответствующий игрок должен быть перемещен в меню, и удален из лобби, с сообщением об ошибке "Вы исключены из лобби".
...
7.Переход между экранами во время игры

    7.1 Визуальное оформление
    * Переход между экранами - короткая тропа через лес.
    * Переход визуально должен выглядеть плавно.

    7.2 Функциональные требования
    * Вся игра происходит на одной сцене.
    * Экраны Арена фарма, Арена, магазин предметов представляют из себя разные части одной сцены.
    * Игрок не должен видеть явного перехода между сценами.
